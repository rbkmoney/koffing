...
export class HttpClaimService {
    ...
    public getClaims(status: string): Promise<Claim[]> {
        const searchParams = new URLSearchParams();
        searchParams.set('claimStatus', status);
    ...
    public createClaim(changeSet: PartyModification[]): Promise<any> {
        return this.http.post(this.claimUrl, changeSet)
            .toPromise()
            .then((response) => response.json());
    }
